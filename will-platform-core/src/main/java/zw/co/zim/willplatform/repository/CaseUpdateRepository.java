...
public interface CaseUpdateRepository extends JpaRepository<CaseUpdate, Long> {
    List<CaseUpdate> findAllByStatusNot(Pageable pageable, RecordStatus status);

    List<CaseUpdate> findAllByCaseIdAndStatusNot(Pageable pageable, Cases caseId, RecordStatus status);
